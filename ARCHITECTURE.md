...
| Data | host bind mount `./data` at `/data` | host bind mount `/srv/bookmarks/data` at `/data` |
...
(`/srv/bookmarks/data`, overridable with `BOOKMARKS_DATA_DIR`) so the database has a stable
